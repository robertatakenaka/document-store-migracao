...
class TestDocumentPipe(unittest.TestCase):
    ...
    def test_identify_data(self):
        ...
        self.assertEqual(
            self.document.a_names,
            {"tab01": (nodes[2], [nodes[0], nodes[1]])}
            )
        self.assertEqual(
            self.texts,
    ...
    def _assert(self, expected, step, xpath=".//a"):
        ...
        for i, node in enumerate(self.xml.findall(xpath)):
            with self.subTest(step + " " + str(i)):
                self.assertEqual(
                    etree.tostring(node),
                    etree.tostring(expected_node[i]))
...
class TestConversionToAnnex(unittest.TestCase):
    ...
    def test_convert_to_app(self):
        ...
        <p><a name="anx01" id="anx01" xml_tag="app" xml_reftype="app" xml_id="anx01"/></p>
        <p><img src="/img/revistas/trends/v33n3/a05tab01.jpg" xml_tag="app" xml_reftype="app" xml_id="anx01"/></p>
        </root>"""
        )
...
class TestConversionToFig(unittest.TestCase):
    ...
    def test_convert_to_figure(self):
        ...
        self.assertIn(
            b'<a name="fig01en" id="fig01en" xml_tag="fig" xml_reftype="fig" xml_id="fig01en"/>',
            _xml
            )
        self.assertIn(
            b'<img src="/img/revistas/jped/v86n3/en_a05fig01.gif" xml_tag="fig" xml_reftype="fig" xml_id="fig01en"/>',
            _xml
            )
