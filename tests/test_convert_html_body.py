...
class TestHTML2SPSPipeline(unittest.TestCase):
    ...
    def test_remove_invalid_br_pipe_removes_br_at_the_beginning(self):
        text = """<root><p><br/> texto E</p></root>"""
        expected = b"""<root><p> texto E</p></root>"""
        xml = etree.fromstring(text)
        text, xml = self.pipeline.RemoveInvalidBRPipe().transform((text, xml))
        self.assertEqual(etree.tostring(xml), expected)

    def test_remove_invalid_br_pipe_removes_br_at_the_end(self):
        text = """<root><p>texto F<br/> </p></root>"""
        expected = b"""<root><p>texto F </p></root>"""
        xml = etree.fromstring(text)
        text, xml = self.pipeline.RemoveInvalidBRPipe().transform((text, xml))
        self.assertEqual(etree.tostring(xml), expected)

    def test_br_pipe_converts_br_into_break(self):
        text = """<root><td>texto A <br/> texto B</td></root>"""
        expected = b"""<root><td>texto A <break/> texto B</td></root>"""
        xml = etree.fromstring(text)
        text, xml = self.pipeline.BRPipe().transform((text, xml))
        self.assertEqual(etree.tostring(xml), expected)

    def test_br_pipe_keeps_br(self):
        text = """<root><sec>texto C <br/> texto D</sec></root>"""
        expected = b"""<root><sec>texto C <br/> texto D</sec></root>"""
        xml = etree.fromstring(text)
        text, xml = self.pipeline.BRPipe().transform((text, xml))
        self.assertEqual(etree.tostring(xml), expected)

    def test_br2p_pipe_create_paragraphs(self):
        text = """<root>
        <p align="x">bla
        <br/> continua outra linha <italic>itálico</italic>
        <br/> linha 2
        </p>
        </root>"""
        expected = b"""<root>
        <p>bla
        </p><p> continua outra linha <italic>it&#225;lico</italic>
        </p><p> linha 2
        </p></root>"""
        xml = etree.fromstring(text)
...
class TestFnIdentifyLabelAndPPipe(unittest.TestCase):
    ...
    def test__create_label__does_not_create_label(self):
        ...
        self.assertIsNone(new_fn.find("label"))
    ...
    def test__identify_label_and_p__identifies_p(self):
        ...
        node = xml.find(".//fn")
        self.assertEqual(node.find("p").text, "TEXTO NOTA")
        self.assertEqual(etree.tostring(xml), expected)
    ...
    def test__identify_label_and_p__creates_p_with_italic(self):
...
class TestFnFixContentPipe(unittest.TestCase):
    def setUp(self):
        self.html_pl = HTML2SPSPipeline(pid="pid")
        self.pl = ConvertElementsWhichHaveIdPipeline()
        self.pipe = self.pl.FnFixContentPipe()
...
class TestFnAddContentPipe(unittest.TestCase):
    ...
    def test__add_fn_tail_into_fn__moves_italic(self):
        text = """<root><fn id="nt01"/>
        ...
        xml = etree.fromstring(text)
        node = xml.find(".//fn")
        self.pipe._add_fn_tail_into_fn(node)
        self.assertIn("Isso", node.find("italic").text)

    def test__add_fn_tail_into_fn__moves_text_children(self):
        text = """<root><fn id="nt01"/>**
           <italic>Isso é conhecido pelos pesquisadores como</italic>
         </root>"""
        expected = """<root><fn id="nt01">**
    ...
    def test__add_fn_tail_into_fn__moves_text_children_tail(self):
        text = """<root><fn id="nt01"/>**
           <italic>Isso é conhecido pelos pesquisadores como</italic> .
         </root>"""
        expected = """<root><fn id="nt01">**
    ...
    def test__add_fn_tail_into_fn__moves_items_until_p_is_found(self):
        text = """<root><fn id="nt01"/>**
           <italic>Isso é conhecido pelos pesquisadores como</italic> .
           <p/> .
         </root>"""
